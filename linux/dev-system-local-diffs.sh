...
if [ "${#FAILOVER[@]}" -gt "0" ]; then
    ...
    lk_echoc "Local $(lk_plural "${#FAILOVER[@]}" file files) with system failover $(lk_plural "${#FAILOVER[@]}" counterpart counterparts):" "$LK_BOLD"
...
if [ "${#NO_SYSTEM[@]}" -gt "0" ]; then
    ...
    lk_echoc "Local $(lk_plural "${#NO_SYSTEM[@]}" file files) with no system $(lk_plural "${#NO_SYSTEM[@]}" counterpart counterparts):" "$LK_BOLD"
